...
class ImageRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # TCL Layer Replacement Initialization
        #print("Starting TCL")
        if opt['network_g']['inp_channels'] == 6:
    ...
    def pad_test(self, window_size):
        ...
        self.lq = self.lq[:, :, 0:h - mod_pad_h * scale, 0:w - mod_pad_w * scale]
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if 'pad' in dataset_name:
                self.pad_test(window_size=8)
            else:
                self.test()
            ...
            if 'dpdd16' in dataset_name:
                sr_img = tensor2img([visuals['result']], out_type=np.uint16, rgb2bgr=rgb2bgr)
            else:
                sr_img = tensor2img([visuals['result']], rgb2bgr=rgb2bgr)
            if 'gt' in visuals:
                if 'dpdd16' in dataset_name:
                    gt_img = tensor2img([visuals['gt']], out_type=np.uint16, rgb2bgr=rgb2bgr)
                else: 
                ...
                if sr_img.shape[2] == 6:
                    ...
                    if self.opt['is_train']:
                        ...
                        save_img_path = osp.join(
                            self.opt['path']['visualization'], dataset_name, 'output',
                            f'{img_name}.png')
                        save_gt_img_path = osp.join(
                            self.opt['path']['visualization'], dataset_name, 'gt',
                            f'{img_name}_gt.png')
